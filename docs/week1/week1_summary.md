...
表中的 E2E 是模型加载完成后的单次推理阶段耗时，不含模型加载；W1 没有正式质量评分器，因此只报告性能和运行状态。

| 任务 | 模型 | 成功运行 | 正确率 | 平均 TTFT | 平均 E2E | 平均 decode | 平均生成 tokens |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| W1 | BitNet | 6/6 | 未评分 | 56.50 ms | 18.509 s | 20.873 tok/s | 384.50 |
| W1 | Qwen | 6/6 | 未评分 | 93.17 ms | 22.280 s | 21.726 tok/s | 481.83 |
| W2 | BitNet | 30/30 | 1/30（3.3%） | 292.40 ms | 6.173 s | 20.969 tok/s | 123.27 |
| W2 | Qwen | 30/30 | 8/30（26.7%） | 129.07 ms | 6.090 s | 21.306 tok/s | 127.00 |
| W3 | BitNet | 30/30 | 4/30（13.3%） | 1642.40 ms | 3.489 s | 20.692 tok/s | 38.50 |
| W3 | Qwen | 30/30 | 0/30（0%） | 471.07 ms | 4.951 s | 21.207 tok/s | 95.00 |

### Deadline 结果

括号内为在 deadline 内完成且答案/action 正确的数量。

| 任务与模型 | 2 s | 5 s | 10 s | 20 s | 30 s |
| --- | ---: | ---: | ---: | ---: | ---: |
| W2 BitNet | — | 3/30（0） | 30/30（1） | 30/30（1） | 30/30（1） |
| W2 Qwen | — | 0/30（0） | 30/30（8） | 30/30（8） | 30/30（8） |
| W3 BitNet | 0/30（0） | 26/30（4） | 30/30（4） | — | — |
| W3 Qwen | 0/30（0） | 28/30（0） | 30/30（0） | — | — |
