...
def _extract_assistant_completion(text: str) -> str:
    if _ASSISTANT_MARKER in text:
        text = text.rsplit(_ASSISTANT_MARKER, 1)[1]
...
def read_generation_text(token_timestamps_path: Path, output_text_path: Path) -> tuple[str, str]:
    if token_timestamps_path.exists():
        ...
        if pieces:
            return _extract_assistant_completion("".join(pieces)), "token_timestamps"

    if output_text_path.exists():
        text = output_text_path.read_text(encoding="utf-8", errors="replace")
        return _extract_assistant_completion(text), "outputs_txt"
